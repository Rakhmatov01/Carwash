          ...
          <aside className="w-[500px] shrink-0 mt-6">
            <div className="sticky top-6">
              <div className="h-[calc(100vh-6rem)] bg-white border rounded-xl shadow-md flex flex-col">
                <h1 className="text-2xl font-bold p-4 border-b">Comments</h1>

                <div className="flex-1 overflow-y-auto p-4">
                  {comments &&
                    comments.map((c, i) => <CommentBox key={i} data={c} />)}
                </div>

                <div className="p-4">
                  <form
                    onSubmit={handleSubmit}
                    className="flex border rounded-lg focus-within:ring-2 focus-within:ring-blue-500"
                  >
                    <input
                      value={usersComment}
                    ...
                    />
                    <button className=" text-white p-3 bg-blue-500 rounded-r-lg">
                      Send
                    </button>
                  </form>
                </div>
...
function CommentBox({ data }) {
  return (
    <div className="border rounded-lg border-gray-200 mb-4">
