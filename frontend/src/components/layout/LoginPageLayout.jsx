...
export default function LoginPageLayout({children, title}){
    const navigate = useNavigate();
    const untitle = title==="register" ? "login" : "register";
    const Title = title[0].toUpperCase()+title.slice(1);
    return (<div className="min-h-dvh flex items-center justify-center px-4 bg-gray-100">
        <div className="w-full max-w-sm rounded-2xl border bg-white p-6 shadow-md flex flex-col">
            ...
            {children}
            <button className="text-white shadow-md text-xl mt-5 px-3 py-2 rounded-xl bg-green-500 active:mt-6 active:shadow-none active:scale-95">{Title}</button>
